...
void MeasDataClass::SetOPUParameters(double AzimuthStart, double AzimuthStop, double AzimuthNumber, double ElevationStart, double ElevationStop, double ElevationNumber, int Speed, int MoveMode)
{
    ...
    MeasArray = QVector<std::complex<double>>(VNAParameters.NumOfPoi * AzimuthNumber * ElevationNumber, std::complex<double>(0.0,0.0));
...
void MeasDataClass::Resize(int FreqNum, int AzNum, int ElNum)
{
    ...
    MeasArray.resize(FreqNum*AzNum*ElNum);

}

std::complex<double> MeasDataClass::ReadFromMeasArray(int FreqIndex, int AzIndex, int ElIndex)
{
    // index out of range?
    return MeasArray[ElIndex * (OPUParameters.AzTrigPoints * VNAParameters.NumOfPoi)  + AzIndex * VNAParameters.NumOfPoi + FreqIndex];
}

void MeasDataClass::WriteValueToArray(int f, int AzIndex, int ElIndex, std::complex<double> NewValue)
{

    // index out of range?
    MeasArray[ElIndex * (OPUParameters.AzTrigPoints * VNAParameters.NumOfPoi)  + AzIndex * VNAParameters.NumOfPoi + f] = NewValue;
}
...
QVector<std::complex<double>> MeasDataClass::GetSweepAt(int AzIndex, int ElIndex)
{
    ...
    {
        SingleSweep[f] = ReadFromMeasArray(f,AzIndex,ElIndex);
    }
    return SingleSweep;
}

void MeasDataClass::GetSweepAt(int AzIndex, int ElIndex, QVector<std::complex<double> >)
{

}

void MeasDataClass::GetSweepAt(int AzIndex, int ElIndex, double *re, double *im)
{
...
void MeasDataClass::WriteSweepToArray(int AzIndex, int ElIndex, QVector <std::complex<double>> SingleSweep)
{
    //Добавить проверку размерности

    for(int f=0; f<VNAParameters.NumOfPoi; f++)
    {
        WriteValueToArray(f,AzIndex,ElIndex,SingleSweep[f]);
    }
}
...
void MeasDataClass::DownSample(int NResultSamples)
{
...
QDataStream &operator<<(QDataStream &out, const MeasDataClass &MyMF)
{
    ...
    QVector<double> RealValuesOfObjectMeasurement(MyMF.MeasArray.size());
    QVector<double> ImagValuesOfObjectMeasurement(MyMF.MeasArray.size());

    for (int i = 0; i < MyMF.MeasArray.size(); ++i)
    {
        RealValuesOfObjectMeasurement[i] = MyMF.MeasArray[i].real();
        ImagValuesOfObjectMeasurement[i] = MyMF.MeasArray[i].imag();
    }
/*
...
QDataStream &operator>>(QDataStream &in, MeasDataClass &MyMeasDataClass)
{
    ...
    MyMeasDataClass.MeasArray.resize(Size);
    for (int i = 0; i < Size; ++i)
    {
        MyMeasDataClass.MeasArray[i] = std::complex<double>(RealValuesOfObjectMeasurement[i], ImagValuesOfObjectMeasurement[i]);
    }
